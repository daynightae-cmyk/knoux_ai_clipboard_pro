...
import { DEVELOPER_TOOLS, DeveloperToolId, getDeveloperToolSample, runDeveloperTool } from "../services/developerTools";
...
export default function StudioPage({ items = [] }: Props) {
  ...
  const report = useMemo(() => ({
    ...
    developerTools: DEVELOPER_TOOLS.map((tool) => ({ id: tool.id, title: tool.title, status: tool.status })),
        ...
        <div className="relative grid grid-cols-1 2xl:grid-cols-[1fr_520px] gap-6 items-end">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Services", value: PRODUCTION_SERVICES.length, icon: Database },
      ...
      <section className="glass-elevated p-5 md:p-6 space-y-5">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-knoux-muted-text max-w-4xl">Run the local utility, load a valid service-specific sample, or copy the last output. No placeholder-only card remains in the developer section.</p>
          </div>
                ...
                <article key={tool.id} className={`relative overflow-hidden rounded-3xl border p-4 min-h-[238px] flex flex-col justify-between gap-4 transition ${selected ? "border-knoux-purple/40 bg-white/88 shadow-knoux-glow-lg" : "border-knoux-purple/12 bg-white/68 shadow-knoux-glow hover:border-knoux-purple/30"}`}>
                  ...
                  <div className="relative space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`knoux-badge ${badgeClass(tool.status)} shrink-0`}>{tool.status}</span>
                    </div>
                  ...
                  <div className="relative grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <button onClick={() => runTool(tool.id)} className="btn-knoux-primary text-[11px]"><Play className="w-3.5 h-3.5" /> {tool.actionLabel}</button>
          ...
          <aside className="space-y-4 2xl:sticky 2xl:top-4 h-fit">
            <div className="rounded-3xl border border-knoux-purple/15 bg-white/75 p-5 shadow-knoux-glow space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <button onClick={() => runTool(currentTool.id)} className="btn-knoux-primary text-xs"><Play className="w-4 h-4" /> Run</button>
