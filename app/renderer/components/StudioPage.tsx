...
export default function StudioPage({ items = [] }: Props) {
  ...
  const [toolOutput, setToolOutput] = useState("Developer utility output appears here.");
  ...
  const report = useMemo(() => ({
    ...
    developerTools: DEVELOPER_TOOLS.map((tool) => ({ id: tool.id, status: tool.status })),
    services: { total: PRODUCTION_SERVICES.length, active: activeServices, ready: readyServices, guarded: guardedServices },
    records: items.length,
    secureRecords: secure,
    pinnedRecords: pinned,
    generatedAt: new Date().toISOString()
  }), [items.length, secure, pinned, activeServices, readyServices, guardedServices]);
  ...
  const runTool = () => {
    const output = runDeveloperTool(toolId, toolInput);
    setToolOutput(output);
    setStatus(`${currentTool.title} completed`);
  };
        ...
        <div className="relative space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              { label: "Secure", value: secure, icon: ShieldCheck },
              { label: "Pinned", value: pinned, icon: CheckCircle2 },
              { label: "Active Services", value: activeServices, icon: Hammer },
              { label: "AI Route", value: api?.status || "check", icon: Sparkles }
            ].map((m) => { const Icon = m.icon; return <div key={m.label} className="knoux-premium-card p-4"><div className="flex items-center justify-between text-[10px] text-knoux-muted-text font-black uppercase"><span>{m.label}</span><Icon className="w-4 h-4 text-knoux-purple" /></div><div className="text-xl font-black text-knoux-dark-text font-mono mt-2 truncate">{m.value}</div></div>; })}
          </div>
        </div>
      </section>
        ...
        <main className="space-y-5">
          ...
          <section className="glass-panel p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-4">
              <div className="grid grid-cols-1 gap-2">{DEVELOPER_TOOLS.map((tool) => <button key={tool.id} onClick={() => setToolId(tool.id)} className={`p-4 rounded-2xl border text-left transition ${toolId === tool.id ? "bg-knoux-purple text-white border-knoux-purple" : "bg-white/75 text-knoux-dark-text border-knoux-purple/10 hover:border-knoux-purple/30"}`}><div className="text-xs font-black">{tool.title}</div><div className="text-[10px] opacity-75 mt-1">{tool.status} · {tool.description}</div></button>)}</div>
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase text-knoux-muted-text">{currentTool.inputLabel}</label>
                <textarea value={toolInput} onChange={(e) => setToolInput(e.target.value)} placeholder={currentTool.placeholder} className="w-full min-h-[150px] rounded-2xl border border-knoux-purple/10 bg-white/80 p-4 text-xs font-mono text-knoux-dark-text outline-none focus:border-knoux-purple" />
                <div className="flex flex-wrap gap-2"><button onClick={runTool} className="btn-knoux-primary text-xs"><Play className="w-4 h-4" /> {currentTool.actionLabel}</button><button onClick={() => copy(toolOutput)} className="btn-knoux-secondary text-xs"><Copy className="w-4 h-4" /> Copy Output</button></div>
                <pre className="rounded-3xl bg-[#140b25] text-[#f7f2ff] p-4 text-[11px] overflow-auto max-h-[300px] whitespace-pre-wrap">{toolOutput}</pre>
              </div>
            </div>
          </section>
