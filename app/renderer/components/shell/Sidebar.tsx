    ...
    <motion.aside id="shell-sidebar-container" animate={{ width: collapsed ? 78 : 280 }} transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }} className="h-screen sticky top-0 flex flex-col justify-between border-r border-white/10 bg-[#12081F]/75 backdrop-blur-2xl select-none shrink-0 z-20 shadow-[18px_0_70px_rgba(0,0,0,0.34)]">
      <div>
        <div className="h-20 flex items-center justify-between px-4 border-b border-white/10 bg-white/[0.03]">
          ...
          <button onClick={() => setCollapsed(!collapsed)} className="w-9 h-9 rounded-xl flex items-center justify-center text-[#BFA7DB] hover:text-white hover:bg-white/10 transition-colors cursor-pointer border border-white/10">{collapsed ? <Menu className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}</button>
        </div>
      ...
      <div className="p-3 border-t border-white/10 bg-gradient-to-b from-transparent to-[#8A2BE2]/10"><div className={`flex items-center gap-3 ${collapsed ? "justify-center" : "px-3 py-3"} rounded-2xl bg-white/[0.05] border border-white/10`}><div className="relative flex items-center justify-center shrink-0"><span className={`absolute inline-flex h-2.5 w-2.5 rounded-full ${privacyMode ? "bg-amber-400" : "bg-emerald-400"} animate-ping opacity-60`} /><span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${privacyMode ? "bg-amber-400" : "bg-emerald-400"}`} /></div>{!collapsed && <div className="flex flex-col text-[10px] overflow-hidden leading-snug"><span className="font-black text-white truncate">{privacyMode ? (ar ? "وضع الخصوصية" : "Privacy Locked") : (ar ? "بيئة آمنة" : "Secure Environment")}</span><span className="text-[#BFA7DB] truncate">{ar ? "الخزنة المحلية نشطة" : "Local Vault Active"}</span></div>}</div>{!collapsed && <div className="text-[9px] text-center text-[#BFA7DB]/55 mt-3 font-mono">A Knoux Product · v1.0.0</div>}</div>
    </motion.aside>
  );
}
