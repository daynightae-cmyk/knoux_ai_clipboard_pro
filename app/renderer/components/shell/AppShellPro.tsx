...
export default function AppShellPro(props: AppShellProProps) {
  return (
    <div className={`relative flex h-screen overflow-hidden bg-gradient-to-tr from-[#FCFAFF] via-[#F7F2FF] to-[#FCFAFF] text-[#241B39] ${props.language === "ar" ? "lang-ar" : ""}`}>
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-28 left-1/4 h-96 w-96 rounded-full bg-[#8A2BE2]/10 blur-3xl" />
        <div className="absolute bottom-0 right-12 h-96 w-96 rounded-full bg-[#D946EF]/10 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col min-w-0 overflow-hidden bg-white/35 backdrop-blur-2xl">
        ...
        <main className="flex-1 overflow-y-auto bg-gradient-to-tr from-transparent via-white/30 to-transparent">{props.children}</main>
      </div>
      {props.toastMessage && <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 p-3.5 rounded-2xl border border-[#8A2BE2]/15 bg-white/95 text-xs text-[#241B39] font-bold shadow-knoux-glow-lg">{props.toastMessage}</div>}
    </div>
  );
}
