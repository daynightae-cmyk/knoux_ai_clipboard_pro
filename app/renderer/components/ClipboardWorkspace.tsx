...
import { buildDailySummary, cleanText, DEFAULT_COLLECTIONS, duplicateSummary, exportCsvFile, exportJsonFile, extractEntities, groupClipsByDate, isElectronRuntime, STATIC_TEMPLATES } from "../services/clientClipboardServices";
...
import { ClipboardBulkResult } from "../workers/workerTypes";
...
interface ClipboardCardProps {
  ...
  checked: boolean;
  isSecureHidden: boolean;
  onCardClick: (item: ClipboardItem) => void;
  onCopyItem: (item: ClipboardItem) => void;
  onToggleFavorite: (item: ClipboardItem) => void;
  onTogglePin: (item: ClipboardItem) => void;
  onDeleteItem: (item: ClipboardItem) => void;
  onRevealToggle: (item: ClipboardItem) => void;
  onSendToAi: (item: ClipboardItem) => void;
}
...
const ClipboardCard = memo(function ClipboardCard({
  ...
  const sensitiveTypes = useMemo(() => detectSensitiveTypes(item.content), [item.content]);
  const domain = useMemo(() => (item.type === "link" ? getDomainLabel(item.content) : null), [item.content, item.type]);

  return (
    <motion.div layout onClick={() => onCardClick(item)} className={`p-4 rounded-2xl bg-white border cursor-pointer group flex items-start justify-between gap-4 knoux-card-hover ${isSelected ? "border-knoux-purple shadow-sm ring-4 ring-knoux-purple/5" : "border-knoux-purple/5 shadow-sm"} ${item.pinned ? "bg-gradient-to-r from-knoux-purple/5 via-white to-white" : ""}`}>
      ...
      <div className="space-y-2 overflow-hidden flex-1">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {sensitiveTypes.length > 0 && <span className="text-[10px] font-extrabold text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded-md">AI Guarded</span>}
          {domain && <span className="text-[10px] font-bold text-sky-700 bg-sky-50 px-1.5 py-0.5 rounded-md">{domain}</span>}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          ...
          {item.language && <span className="text-[9px] text-violet-700 bg-violet-50 px-1.5 py-0.5 rounded font-bold uppercase">{item.language}</span>}
      ...
      <div className="flex items-center gap-1 shrink-0 opacity-80 group-hover:opacity-100 transition-opacity">
        {item.isSecure && <button onClick={(event) => { event.stopPropagation(); onRevealToggle(item); }} className="p-1.5 rounded-lg border border-knoux-purple/5 bg-[#FCFAFF] text-knoux-muted-text hover:text-knoux-purple">{isSecureHidden ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}</button>}
        ...
        <button onClick={(event) => { event.stopPropagation(); onSendToAi(item); }} className="p-1.5 rounded-lg border border-knoux-purple/5 bg-[#FCFAFF] text-knoux-purple"><Sparkles className="w-3.5 h-3.5" /></button>
...
export default function ClipboardWorkspace({ items, onAddNewItem, onCopyItem, onTogglePin, onToggleFavorite, onDeleteItem, onClearHistory, setActiveTab, setAiInputText, searchQuery, onUpdateItems }: ClipboardWorkspaceProps) {
  ...
  const clipboardSearch = useWorker<ClipboardBulkResult, {
    items: ClipboardItem[];
    query: string;
    ...
    channel: "clipboard",
    kind: "clipboard-bulk-process",
    timeoutMs: 6000,
  });
  const { run: runClipboardSearch } = clipboardSearch;
  ...
  const fallbackFilteredItems = useMemo(() => items.filter((item) => {
  ...
  }), [businessMode, debouncedQuery, items, selectedFilter, selectedFolder]);

  useEffect(() => {
    void runClipboardSearch({
      items,
  ...
  }, [businessMode, debouncedQuery, items, runClipboardSearch, selectedFilter, selectedFolder]);

  const filteredItems = (clipboardSearch.result?.items as ClipboardItem[] | undefined) || fallbackFilteredItems;

  const virtual = useVirtualList({ count: filteredItems.length, estimateSize: 154, overscan: 8, storageKey: "knoux-clipboard-scroll" });

  const handleCardClick = useCallback((item: ClipboardItem) => {
    if (isMultiSelect) {
  ...
  const handleCreateSnippet = useCallback((event: React.FormEvent) => {
    event.preventDefault();
    if (!newContent.trim()) return;
    onAddNewItem(newContent, newType, "System Note");
    setNewContent("");
    setIsAddingNew(false);
  }, [newContent, newType, onAddNewItem]);
  ...
  const handleRemoveDuplicates = useCallback(async () => {
    const cleaned = await runWorkerTask<ClipboardBulkResult, { items: ClipboardItem[]; mode: "dedupe" }>({
      kind: "clipboard-bulk-process",
      payload: { items, mode: "dedupe" },
      timeoutMs: 6000,
    }).catch(() => null);
    const nextItems = (cleaned?.items as ClipboardItem[] | undefined) || items;
    onUpdateItems(nextItems);
    setServiceNotice(`Removed ${items.length - nextItems.length} duplicate unpinned clips.`);
  }, [items, onUpdateItems]);
  ...
  }, [items, onUpdateItems, selectedIds]);

  const handleBulkPin = useCallback((pinValue: boolean) => {
    onUpdateItems(items.map((item) => selectedIds.has(item.id) ? { ...item, pinned: pinValue } : item));
    setSelectedIds(new Set());
  }, [items, onUpdateItems, selectedIds]);

  const handleExportJson = useCallback(() => { exportJsonFile("knoux-clipboard-vault.json", { exportedAt: new Date().toISOString(), items }); setServiceNotice("Local clipboard vault exported as JSON."); }, [items]);
  const handleExportCsv = useCallback(() => { exportCsvFile("knoux-visible-clips.csv", filteredItems); setServiceNotice("Visible clip list exported as CSV."); }, [filteredItems]);
  const handleTemplate = useCallback((content: string, label: string) => { onAddNewItem(content, "note", `Template: ${label}`); setServiceNotice(`${label} saved as a copy template.`); }, [onAddNewItem]);
  const sendToAi = useCallback((item: ClipboardItem) => { setAiInputText(item.content); setActiveTab("ai"); }, [setActiveTab, setAiInputText]);
  const handleRevealToggle = useCallback((item: ClipboardItem) => {
    setRevealedSecureId((current) => current === item.id ? null : item.id);
  }, []);
    ...
    <div id="clipboard-workspace-root" className="h-[calc(100vh-64px)] flex flex-col select-none">
      <div className="px-4 pt-4 bg-transparent">
        <div className="knoux-section p-4 space-y-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-[10px] font-black"><span className="knoux-badge knoux-badge-active">{runtimeLabel}</span><span className="knoux-badge knoux-badge-ready">Virtualized {filteredItems.length} records</span>{clipboardSearch.loading && <span className="knoux-badge knoux-badge-ready">Worker filtering...</span>}</div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main ref={virtual.containerRef} onScroll={virtual.onScroll} className="flex-1 overflow-y-auto p-4 bg-[#FCFAFF]">
          ...
          {filteredItems.length === 0 ? <div className="h-64 flex flex-col items-center justify-center text-center p-8 border border-dashed border-knoux-purple/15 rounded-2xl bg-white/40"><FileText className="w-10 h-10 text-knoux-purple/30 mb-2" /><span className="text-xs font-bold text-knoux-dark-text">No records found matching filters</span><p className="text-[10px] text-knoux-muted-text mt-1 max-w-xs">Modify search parameters or click New Snippet above.</p></div> : <div style={{ height: virtual.totalSize, position: "relative" }}>{virtual.virtualItems.map((row) => { const item = filteredItems[row.index]; if (!item) return null; return <div key={item.id} style={{ position: "absolute", top: row.start, left: 0, right: 0, height: row.size, paddingBottom: 12 }}><ClipboardCard item={item} isSelected={selectedItem?.id === item.id} isMultiSelect={isMultiSelect} checked={selectedIds.has(item.id)} isSecureHidden={item.isSecure && revealedSecureId !== item.id} onCardClick={handleCardClick} onCopyItem={onCopyItem} onToggleFavorite={onToggleFavorite} onTogglePin={onTogglePin} onDeleteItem={onDeleteItem} onRevealToggle={handleRevealToggle} onSendToAi={sendToAi} /></div>; })}</div>}
        </main>
