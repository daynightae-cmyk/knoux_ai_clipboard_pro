...
import { Camera, ImageUp, QrCode, Copy, CheckCircle, AlertTriangle, Keyboard, StopCircle, RefreshCw, ClipboardImage } from "lucide-react";
...
export default function BarcodeScannerPage({ onAddNewItem }: BarcodeScannerPageProps) {
  ...
  const setMessage = (message: string, tone: ScanStatus = "ready") => { setStatus(message); setStatusTone(tone); };
  ...
  useEffect(() => { refreshCameras(); return () => stopCamera(); }, []);
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      const controls = typeof reader.decodeFromConstraints === "function"
        ? await reader.decodeFromConstraints(constraints, video, onFrame)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6">
        ...
        <aside className="space-y-4">
          <div className="glass-panel p-4 space-y-3">
            <div className="text-xs font-black uppercase tracking-widest text-knoux-muted-text">Camera</div>
            <select value={deviceId} onChange={(e) => setDeviceId(e.target.value)} className="w-full h-11 rounded-2xl border border-knoux-purple/10 bg-white p-2 text-xs text-knoux-dark-text outline-none focus:border-knoux-purple/40">
              <option value="">Auto / Environment camera</option>
              {devices.map((device, index) => <option key={device.deviceId || index} value={device.deviceId}>{device.label || `Camera ${index + 1}`}</option>)}
            </select>
            <button onClick={refreshCameras} className="w-full h-10 rounded-xl border border-knoux-purple/10 bg-white text-knoux-purple text-xs font-bold flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4" /> Refresh Cameras</button>
          </div>
