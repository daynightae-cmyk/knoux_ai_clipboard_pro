  ...
  const [enabledServices, setEnabledServices] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(
      PRODUCTION_SERVICES.map((service) => [
        service.id,
        service.implemented && service.status !== "Disabled",
      ])
    )
  );
  ...
  return (
    <div id="labs-workspace-container" className="p-6 space-y-6 max-w-6xl mx-auto select-none">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-[1.25fr_0.75fr] gap-6 items-center">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-knoux-muted-text leading-relaxed max-w-2xl">
                This page only shows registry state for active, guarded, and planned services.
                Planned capabilities stay disabled until real handlers and packaging are verified.
              </p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
              {[
                ...
                {
                  label: "OpenRouter",
                  value: `${PRODUCTION_SCORE.openRouterBridge}%`,
                  icon: Sparkles,
                },
                {
                  label: "Security",
                  value: `${PRODUCTION_SCORE.securityVault}%`,
                  icon: ShieldCheck,
                },
              ].map((metric) => {
                ...
                return (
                  <div
                    key={metric.label}
                    className="rounded-2xl border border-knoux-purple/10 bg-white/80 p-3 shadow-sm"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] uppercase tracking-wider font-bold text-knoux-muted-text">
                        {metric.label}
                      </span>
                      <Icon className="w-4 h-4 text-knoux-purple" />
                    </div>
                    <div className="text-xl font-black text-knoux-dark-text font-mono mt-2">
                      {metric.value}
                    </div>
                  </div>
                );
          ...
          <div className="rounded-3xl border border-knoux-purple/10 bg-white/80 p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-2 text-[11px] text-knoux-muted-text leading-relaxed">
              <p>
                <strong className="text-knoux-dark-text">Vercel:</strong> uses web-only renderer
                build and /api/ai/[action].
              </p>
              <p>
                <strong className="text-knoux-dark-text">Electron:</strong> uses preload-safe IPC
                channels and OpenRouter helper.
              </p>
              <p>
                <strong className="text-knoux-dark-text">Fallback:</strong> explicit local
                deterministic result, never fake live success.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-5">
        {PRODUCTION_SERVICES.filter((service) =>
          ["Guarded", "Planned", "Missing", "Disabled"].includes(service.status)
        ).map((service, index) => (
          <motion.div
            key={service.id}
          ...
          >
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <span
                    className={`text-[9px] font-black border px-2 py-0.5 rounded-full uppercase tracking-wider ${tierStyle[service.tier]}`}
                  >
                    {service.tier}
                  </span>
                  <button
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-knoux-muted-text uppercase tracking-wider">
                    {service.status}
                  </span>
                </div>
            ...
            <div className="mt-4 pt-3 border-t border-knoux-purple/5 flex items-center justify-between gap-3">
              <span className="text-[10px] text-knoux-muted-text font-mono truncate">
                {service.channel || "internal"}
              </span>
              <span
                className={`text-[10px] font-black ${service.status === "Active" ? "text-emerald-700" : service.status === "Guarded" ? "text-amber-700" : service.status === "Ready" ? "text-blue-700" : "text-slate-500"}`}
              >
                {service.status === "Active"
                  ? "Action available"
                  : service.status === "Ready"
                    ? "Ready"
                    : service.status === "Guarded"
                      ? "Guarded"
                      : service.status}
              </span>
            </div>
        ...
        {[
          {
            ...
            value: `${PRODUCTION_SCORE.openRouterBridge}%`,
            icon: Brain,
            note: "Live API + Electron helper + fallback",
          },
          {
            title: "Storage Path",
            value: `${PRODUCTION_SCORE.sqlitePersistence}%`,
            icon: Database,
            note: "Memory adapter ready; SQLite hard persistence next",
          },
          {
            title: "Experience Layer",
            value: `${PRODUCTION_SCORE.serviceTransparency}%`,
            icon: Globe2,
            note: "Clear status instead of placeholder success",
          },
        ].map((item) => {
          ...
          return (
            <div
              key={item.title}
              className="rounded-3xl border border-knoux-purple/10 bg-white p-5 shadow-sm space-y-3"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-black uppercase tracking-wider text-knoux-dark-text">
                  {item.title}
                </span>
