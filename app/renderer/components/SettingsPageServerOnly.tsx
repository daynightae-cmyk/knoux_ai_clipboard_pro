    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
        ...
        <main className="space-y-5">
          ...
          <div className="glass-panel p-5 space-y-4"><h3 className="text-xs font-black uppercase flex items-center gap-2 text-knoux-dark-text"><Layout className="w-4 h-4 text-knoux-purple" /> Appearance</h3><div className="space-y-2"><div className="text-[10px] font-black uppercase text-knoux-muted-text">Theme Mode</div><div className="flex flex-wrap gap-2">{(["light", "dark", "system"] as const).map((themeMode) => <button key={themeMode} onClick={() => update({ themeMode })} className={`px-4 py-2 rounded-xl text-xs font-bold capitalize ${settings.themeMode === themeMode ? "bg-knoux-purple text-white" : "bg-white text-knoux-purple border border-knoux-purple/10"}`}>{themeMode}</button>)}</div></div><div className="space-y-2"><div className="text-[10px] font-black uppercase text-knoux-muted-text">UI Density</div><div className="flex flex-wrap gap-2">{(["compact", "comfortable", "spacious"] as const).map((density) => <button key={density} onClick={() => update({ density })} className={`px-4 py-2 rounded-xl text-xs font-bold capitalize ${settings.density === density ? "bg-knoux-purple text-white" : "bg-white text-knoux-purple border border-knoux-purple/10"}`}>{density}</button>)}</div></div><div className="space-y-2"><div className="text-[10px] font-black uppercase text-knoux-muted-text">Glow / Transparency Accent</div><div className="flex flex-wrap gap-2">{(["low", "medium", "high"] as const).map((g) => <button key={g} onClick={() => update({ glowIntensity: g })} className={`px-4 py-2 rounded-xl text-xs font-bold capitalize ${settings.glowIntensity === g ? "bg-knoux-purple text-white" : "bg-white text-knoux-purple border border-knoux-purple/10"}`}>{g}</button>)}</div></div></div>

          <div className="glass-panel p-5 space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-xs font-black uppercase flex items-center gap-2 text-knoux-dark-text"><ServerCog className="w-4 h-4 text-knoux-purple" /> AI Provider Diagnostics</h3><p className="text-[11px] text-knoux-muted-text mt-1 max-w-2xl">{ar ? "التشغيل يعتمد على مسار السيرفر فقط." : "AI execution is routed through the server endpoint only."}</p></div><span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase border ${providerTone}`}>{providerStatus?.status || "provider_not_checked"}</span></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-[11px] text-amber-900 flex gap-2 leading-relaxed"><AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" /><span>{ar ? "إذا كان مزود السيرفر غير مضبوط سيظهر provider_not_configured بوضوح." : "If the server provider is missing, the app reports provider_not_configured honestly."}</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[11px]"><div className="rounded-2xl border border-knoux-purple/10 bg-white/70 p-3"><b>Status</b><br /><span className="text-knoux-muted-text">{providerStatus?.status || "not checked"}</span></div><div className="rounded-2xl border border-knoux-purple/10 bg-white/70 p-3"><b>Model</b><br /><span className="text-knoux-muted-text font-mono">{providerStatus?.model || "server default"}</span></div><div className="rounded-2xl border border-knoux-purple/10 bg-white/70 p-3"><b>Runtime</b><br /><span className="text-knoux-muted-text">server-route-only</span></div></div><button onClick={checkProvider} disabled={checkingProvider} className="px-4 py-2 rounded-xl bg-knoux-purple text-white text-xs font-bold disabled:opacity-60 flex items-center gap-2"><RefreshCw className={`w-4 h-4 ${checkingProvider ? "animate-spin" : ""}`} />{checkingProvider ? "Checking..." : "Check Server Provider"}</button></div>

          <div className="glass-panel p-5 space-y-4"><h3 className="text-xs font-black uppercase flex items-center gap-2 text-knoux-dark-text"><Sparkles className="w-4 h-4 text-knoux-purple" /> AI & Storage</h3><button onClick={() => update({ autoAnalyze: !settings.autoAnalyze })} className="px-4 py-2 rounded-xl bg-knoux-purple/10 text-knoux-purple text-xs font-bold">Auto Analyze: {settings.autoAnalyze ? "ON" : "OFF"}</button><button disabled title="Cloud sync is disabled until a real sync backend exists." className="px-4 py-2 rounded-xl bg-slate-100 text-slate-500 text-xs font-bold cursor-not-allowed">Cloud Sync: Guarded</button><div className="space-y-2"><span className="text-xs font-bold text-knoux-dark-text">History limit: {settings.maxHistorySize}</span><input type="range" min="20" max="250" step="10" value={settings.maxHistorySize} onChange={(e) => update({ maxHistorySize: Number(e.target.value) })} className="w-full accent-knoux-purple" /></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2">{PRODUCTION_SERVICES.slice(0, 4).map((s) => <div key={s.id} className="rounded-2xl border border-knoux-purple/10 bg-white/70 p-3 text-[11px] text-knoux-dark-text"><b>{s.displayName}</b><br /><span className="text-knoux-muted-text">{s.status}</span></div>)}</div></div>
          <div className="glass-panel p-5 space-y-3"><h3 className="text-xs font-black uppercase flex items-center gap-2 text-knoux-dark-text"><Database className="w-4 h-4 text-knoux-purple" /> Backup</h3><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><button onClick={exportJson} className="h-10 rounded-xl border border-knoux-purple/10 text-knoux-purple text-xs font-bold flex items-center justify-center gap-2 bg-white"><Download className="w-4 h-4" /> Export JSON</button><label className="h-10 rounded-xl border border-knoux-purple/10 text-knoux-purple text-xs font-bold flex items-center justify-center gap-2 cursor-pointer bg-white"><Upload className="w-4 h-4" /> Import JSON<input type="file" accept=".json" onChange={importJson} className="hidden" /></label></div><p className="text-[10px] text-knoux-muted-text">Provider runtime configuration is excluded from backup exports.</p></div>
