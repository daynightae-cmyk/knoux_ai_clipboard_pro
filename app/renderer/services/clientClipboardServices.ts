...
export function autoTags(content: string): string[] {
  ...
  if (detectSensitiveTypes(text).length) tags.add("secret");
...
export function extractEntities(content: string): ExtractedEntities {
  ...
  const possibleAddresses = content
    .split(/\n|;/)
    .map((line) => line.trim())
    .filter((line) => /\b(street|st\.|road|rd\.|avenue|ave\.|abu dhabi|dubai|uae|zip|postal)\b/i.test(line))
    .slice(0, 8);
  return {
    emails: Array.from(new Set(emails)),
    phones: Array.from(new Set(phones)),
    urls: Array.from(new Set(urls)),
    possibleAddresses: Array.from(new Set(possibleAddresses)),
  };
}
...
export function duplicateSummary(items: ClipboardItem[]) {
  ...
  return {
    ...
    duplicateCount: duplicateGroups.reduce((sum, count) => sum + count - 1, 0),
  };
}
...
export function buildDailySummary(items: ClipboardItem[]) {
  ...
  const typeCounts = today.reduce<Record<string, number>>((acc, item) => {
  ...
  return {
    ...
    pinned: today.filter((item) => item.pinned).length,
    sensitive: today.filter((item) => item.isSecure || detectSensitiveTypes(item.content).length).length,
    duplicatesAvoided: duplicateSummary(items).duplicateCount,
  };
}
...
export function isElectronRuntime() {
  return Boolean((window as any).electronAPI?.clipboard || (window as any).electron?.clipboard);
}

export async function importCurrentClipboardFromRuntime(): Promise<string> {
  const api = (window as any).electronAPI?.clipboard || (window as any).electron?.clipboard;
...
export const CLIENT_SERVICE_CARDS: ServiceCard[] = [
  ...
  { id: "sensitive-data-guard", displayName: "Sensitive Data Guard", description: "Detects passwords, API keys, tokens, card-like numbers, and private-key text locally.", status: "Active", implemented: true, requiresConfig: false, runtimeType: "shared", dependency: "detectSensitiveTypes", fallback: "AI actions stay guarded for sensitive content.", actionHandler: "detectSensitiveTypes" },
  ...
  { id: "clipboard-collections", displayName: "Clipboard Collections", description: "Work, Personal, Code, Links, Replies, Shipping, Invoices, and Important collections persist locally.", status: "Active", implemented: true, requiresConfig: false, runtimeType: "shared", dependency: "knoux_folders localStorage", fallback: "General collection is always available.", actionHandler: "folder assignment" },
  { id: "smart-labels", displayName: "Smart Labels / Auto Tags", description: "Rule-based tags include email, phone, link, code, json, invoice, tracking, address, and secret.", status: "Active", implemented: true, requiresConfig: false, runtimeType: "shared", dependency: "autoTags", fallback: "Manual tags can be added.", actionHandler: "autoTags" },
  { id: "copy-templates", displayName: "One-Click Copy Templates", description: "Static business templates copy or save real text with no AI dependency.", status: "Active", implemented: true, requiresConfig: false, runtimeType: "shared", dependency: "STATIC_TEMPLATES", fallback: "Templates can be copied manually from the panel.", actionHandler: "STATIC_TEMPLATES" },
