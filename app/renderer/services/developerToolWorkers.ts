...
export const WORKER_SUPPORTED_DEV_TOOLS: DeveloperToolId[] = ["json-format", "regex-test", "markdown-table", "jwt-inspector", "redaction-map"];
...
export async function runDeveloperToolFast(id: DeveloperToolId, input: string): Promise<string> {
  ...
  if (id === "jwt-inspector") return runDeveloperTool(id, input);
  if (id === "redaction-map") {
