...
export class LifecycleManager {
  ...
  private initialize(): void {
    this.llog.info('Initializing lifecycle manager', {
      config: this.config,
    ...
    this.llog.debug('Lifecycle manager initialized');
  }
  ...
  private transitionState(newState: AppLifecycleState): void {
    ...
    this.llog.info('Application state transition', {
      oldState,
      newState,
      sessionId: this.state.sessionId,
  ...
  private onError(): void {
    ...
    this.llog.error('Application entered error state', {
  ...
  private setupErrorHandlers(): void {
    ...
    this.llog.debug('Global error handlers setup completed');
  }
  ...
  private handleError(error: Error, source: string): void {
    this.llog.error(`Application error from ${source}`, error, {
    ...
    setTimeout(() => {
      try {
        ...
        if (this.state.recoveryAttempts < this.recoveryOptions.maxAttempts) {
          ...
          this.llog.error('Maximum recovery attempts reached, giving up');
  ...
  public updateConfig(config: AppConfig): void {
    ...
    if (configChanged) {
      this.llog.info('Lifecycle manager configuration updated', {
  ...
  public forceShutdown(): void {
    this.llog.warn('Initiating force shutdown');
    this.isShuttingDown = true;
    this.stopAllTimers();
    app.exit(1);
  }
  ...
  private notifyStateChangeListeners(oldState: AppLifecycleState, newState: AppLifecycleState): void {
    for (const listener of this.stateChangeListeners) {
      try {
        ...
        this.llog.error('State change listener error', error as Error);
      }
    }
  }
  ...
  private notifyErrorHandlers(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        this.llog.error('Error handler error', handlerError as Error);
      }
    }
  }
